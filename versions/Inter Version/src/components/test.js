...
// ====================== PRESETS ANALYSIS ======================
console.log("🔍 PRESETS ANALYSIS");
console.log("Total Presets:", PRESETS.length);

const categories = {};
const types = {};
const animatedCount = PRESETS.filter((p) => p.animated).length;
...
PRESETS.forEach((p, index) => {
  ...
  if (duplicateIds.has(p.id)) {
    console.warn(`⚠️ Duplicate ID found: ${p.id} at index ${index}`);
  }
...
console.table({
  ...
  Static: PRESETS.length - animatedCount,
});
...
const topNames = [...PRESETS]
  .sort((a, b) => (b.name?.length || 0) - (a.name?.length || 0))
  .slice(0, 10)
  .map((p) => ({ name: p.name, id: p.id, category: p.category }));

console.log("🔝 Top 10 Longest Names:");
console.table(topNames);

// Search helper
globalThis.findPreset = (query) => {
  const results = PRESETS.filter(
    (p) =>
      p.name?.toLowerCase().includes(query.toLowerCase()) ||
      p.id?.toLowerCase().includes(query.toLowerCase()) ||
      p.equation?.toLowerCase().includes(query.toLowerCase()),
  );
  console.log(`Found ${results.length} presets for "${query}":`);
  console.table(
    results.map((p) => ({ id: p.id, name: p.name, category: p.category })),
  );
  return results;
};
...
// Example usage:
console.log(
  "%cTry in console: findPreset('everest')",
